...
if (isset($cari) && ($cari != "")) {
	...
	$sql = "select * from lahan where name like '%$cari%' ".$order;
	$count_query = "select count(*) from lahan where name like '%$cari%'";
}else{
	$t->set_var("MESSAGE", "Seluruh Data");
	$sql = "select * from lahan ".$order;
	$count_query = "select count(*) from lahan";
}
