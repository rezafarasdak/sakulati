    ...
    if($("#validate").length > 0)
        $("#validate, #validate_custom").validationEngine('attach',
		{
			...
			onValidationComplete: function(form, status){
				if(status){
/*					$('#submitHandling').text('Wait ...');
					$('#submitHandling').attr({disabled: 'disabled'});
*/
					return true;	
				}
			}			
	...
	/* Monthpicker, Month & Year Only, Reza 15 Jan 2016 */
	...
	/* Datepicker, Month & Year Only, Reza 15 Jan 2016 */
    ...
    if($("table.simple_sort").length > 0)
        $("table.simple_sort").dataTable({"iDisplayLength": 25,"bLengthChange": false,"bFilter": false,"bInfo": false,"bPaginate": true});

    if($("table.simple_sort_cwise").length > 0)
        $("table.simple_sort_cwise").dataTable({
			"iDisplayLength": 25,
			"bLengthChange": false,
			"bFilter": false,
			"bInfo": false,
    ...
    if($("table.sort").length > 0)
        $("table.sort").dataTable({"iDisplayLength": 25, "sPaginationType": "full_numbers","bLengthChange": false,"bFilter": false,"bInfo": false,"bPaginate": true, "aoColumns": [ { "bSortable": false }, null, null, null, null]});
    
    if($("table.sortc").length > 0)
        $("table.sortc").dataTable({"iDisplayLength": 25, "aLengthMenu": [25,50,100], "sPaginationType": "full_numbers", "aoColumns": [ { "bSortable": false }, null, null, null, null]});
    /* EOF datatables */    
